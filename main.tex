...
% --- PAQUETES DE TABLAS Y MATEMÁTICAS ---
\usepackage{tabularx}
\usepackage{booktabs}
\usepackage{graphicx} % Para las fotos
\usepackage{amsmath}  % Para que reconozca el comando \text en las medidas
\usepackage{float}    % Para que el [H] fije la tabla en su lugar
\usepackage{caption} % <--- EL SALVADOR DEL ERROR @makecaption
...
Para garantizar la viabilidad técnica del desarrollo y la correcta evaluación del prototipo funcional (Producto Mínimo Viable - MVP) dentro de las restricciones de memoria de la plataforma embebida, el alcance inicial del proyecto se delimita de la siguiente manera:

\begin{itemize}
    \item \textbf{Focalización vehicular:} El sistema de visión artificial y la red neuronal se entrenarán de manera exclusiva para la detección y reconocimiento de \textbf{placas de motocicletas}. Esta delimitación responde a las diferencias morfológicas y geométricas que presentan estas placas frente a las de automóviles convencionales, lo que requiere un enfoque especializado en el \textit{dataset} para asegurar una alta tasa de aciertos en la inferencia local.
    \item \textbf{Entorno de validación:} Aunque el diseño se proyecta para la Sede Tintal, las pruebas de validación del prototipo se ejecutarán en la Sede Centro de la ETITC, simulando las condiciones de iluminación y distancia requeridas.
    \item \textbf{Actuación física:} La validación de acceso exitoso se representará mediante una etapa de señalización lumínica (LED de estado), omitiendo la instalación de barreras mecánicas o talanqueras reales durante esta fase de demostración tecnológica.
\end{itemize}
...
\begin{table}[!ht]
\centering
\caption{Matriz de alineación metodológica: Objetivos, Fases y Actividades (Actualizada)}
...
\midrule
1. Implementar validación mediante sensores y redes neuronales embebidas. & Fase 1: Diseño de Arquitectura y Hardware & A1.1 Selección de la plataforma embebida.\newline A1.2 Selección de la red de sensores.\newline A1.3 Diseño mecánico y ensamblaje profesional (3D/PCB).\newline A1.4 Propuesta de escalabilidad fotovoltaica. \\
\midrule
2. Desplegar modelo de reconocimiento capaz de activar actuador de potencia. & Fase 2: Modelado TinyML y Señalización & A2.1 Estrategia de curaduría y minería de datos masivos.\newline A2.2 Entrenamiento y optimización en Edge Impulse.\newline A2.3 Diseño de la etapa de señalización visual (LED). \\
\midrule
3. Diseñar módulo de gestión de trazabilidad segura y confiable. & Fase 3: Implementación de Trazabilidad & A3.1 Configuración de lista blanca en memoria local.\newline A3.2 Registro de eventos y almacenamiento de logs en SD. \\
\midrule
Validación general del sistema frente a variables dependientes. & Fase 4: Pruebas, Validación y Evaluación & A4.1 Ejecución de pruebas en entorno controlado (Sede Centro).\newline A4.2 Análisis estadístico comparativo vs. sistema manual. \\
\bottomrule
\end{tabularx}
\end{table}
...
Para garantizar la viabilidad del procesamiento de imágenes en el borde, se definieron criterios de evaluación específicos para visión artificial y redes neuronales, descartando parámetros de instrumentación convencional que no son críticos para este prototipo. 

Se evaluaron tres alternativas tecnológicas utilizando una escala cuantitativa de 1 (desempeño bajo) a 5 (desempeño alto), como se detalla en la Tabla \ref{tab:matriz_decision}. Los criterios evaluados fueron:
\begin{itemize}
    \item \textbf{Costo de implementación:} Accesibilidad económica para garantizar escalabilidad.
    \item \textbf{Memoria RAM/PSRAM:} Capacidad para almacenar el \textit{framebuffer} y los tensores del modelo.
    \item \textbf{Hardware de Visión:} Disponibilidad de cámara integrada para evitar latencia de transmisión.
    \item \textbf{Soporte TinyML:} Compatibilidad oficial con plataformas como \textit{Edge Impulse}.
    \item \textbf{Eficiencia Energética:} Consumo de potencia para operar de forma autónoma.
\end{itemize}

\begin{table}[H]
\centering
\caption{Matriz de decisión para la selección de la plataforma embebida}
\label{tab:matriz_decision}
\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\columnwidth}{>{\raggedright\arraybackslash\bfseries}p{0.22\columnwidth} *{5}{>{\centering\arraybackslash}X} >{\centering\arraybackslash\bfseries}X}
\toprule
Plataforma & Costo & Memoria (RAM) & Hardware de Visión & Soporte TinyML & Eficiencia Energética & Puntaje Total \\
\midrule
Raspberry Pi 4 & 1 & 5 & 3 & 4 & 2 & 15 \\
Arduino Nano 33 BLE & 3 & 2 & 2 & 5 & 5 & 17 \\
ESP32-CAM & 5 & 4 & 5 & 5 & 4 & 23 \\
\bottomrule
\end{tabularx}
\end{table}

El análisis cuantitativo evidencia que el \textbf{ESP32-CAM} obtiene el mayor puntaje (23 puntos). Su integración nativa del sensor OV2640, su memoria PSRAM externa y su compatibilidad total para exportar librerías desde \textit{Edge Impulse} lo consolidan como la plataforma idónea, superando el alto consumo de la Raspberry Pi y las limitaciones visuales del Arduino Nano.
...
\subsubsection{Actividad 1.4: Propuesta de escalabilidad energética (Sistema Fotovoltaico)}
Como parte de la proyección de escalabilidad y autonomía del prototipo, se contempla la futura integración de un sistema de alimentación fotovoltaica. Aunque en la fase de Producto Mínimo Viable (MVP) el sistema operará con una fuente de alimentación de corriente continua estándar (5V/2A) debido a restricciones de tiempo y presupuesto, el diseño mecánico y estructural dejará la provisión física para un panel solar.

Este enfoque de nodo autónomo (\textit{Off-Grid}) elimina la necesidad de realizar acometidas eléctricas y obras civiles en los puntos de acceso remotos de la universidad. Para el diseño en software CAD, se dimensionó un panel monocristalino de 20W ($450 \text{ mm} \times 350 \text{ mm} \times 25 \text{ mm}$). Este panel alimentaría un controlador de carga PWM acoplado a una batería de respaldo (por ejemplo, ácido-plomo de 12V 7Ah), garantizando la operación continua del microcontrolador y el sensor durante las horas nocturnas o de baja radiación. 

Para reducir el voltaje de los 12V de la batería a los 5V requeridos por el ESP32-CAM, el circuito integrará un módulo reductor (Step-Down) LM2596. A continuación, en la Tabla \ref{tab:cotizacion_solar}, se presenta una proyección de costos estimada para esta mejora futura:

\begin{table}[H]
\centering
\caption{Proyección de costos para escalabilidad con sistema fotovoltaico autónomo}
\label{tab:cotizacion_solar}
\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\columnwidth}{>{\raggedright\arraybackslash}p{0.45\columnwidth} >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\raggedright\arraybackslash\bfseries}X}
\toprule
Componente & Cantidad & Valor Unitario (COP) & Valor Total (COP) \\
\midrule
Panel Solar Monocristalino (20W, 12V) & 1 & \$ 85,000 & \$ 85,000 \\
Controlador de Carga Solar (PWM 10A) & 1 & \$ 35,000 & \$ 35,000 \\
Batería Seca Recargable (12V, 7Ah) & 1 & \$ 65,000 & \$ 65,000 \\
Módulo Reductor Step-Down LM2596 & 1 & \$ 8,000 & \$ 8,000 \\
Estructura de fijación y cableado & 1 & \$ 25,000 & \$ 25,000 \\
\midrule
\multicolumn{3}{r}{\textbf{Costo Total Estimado}} & \textbf{\$ 218,000} \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Fase 2: Modelado TinyML y Etapa de Señalización}
En esta etapa se desarrolla el núcleo inteligente del sistema, abarcando la consolidación del conjunto de datos (\textit{dataset}), el entrenamiento de la red neuronal y la capacidad de accionar el entorno físico tras una lectura exitosa.

\subsubsection{Actividad 2.1: Estrategia de curaduría y recolección de datos}
Para el entrenamiento del modelo de visión artificial, se estructuró un conjunto de datos (\textit{dataset}) compuesto por 531 imágenes. Para garantizar la robustez de la red neuronal y evitar sesgos, se empleó una estrategia de recolección mixta:
\begin{itemize}
    \item \textbf{Imágenes de repositorio (80\% aprox.):} Se extrajeron alrededor de 430 imágenes etiquetadas desde el repositorio especializado Roboflow Universe (formato YOLOv5 PyTorch). Estas imágenes aportan variabilidad en tipografías, ángulos e iluminación.
    \item \textbf{Imágenes propias (20\% aprox.):} Se capturaron cerca de 100 fotografías reales utilizando un dispositivo móvil de alta resolución. Estas imágenes fueron posteriormente redimensionadas por la plataforma para enseñar a la red a reconocer el entorno real y las texturas locales.
\end{itemize}

% Pega esto justo debajo del texto de la Actividad 2.1
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Data_acquisition.png} 
    \caption{Interfaz de adquisición de datos en Edge Impulse mostrando el etiquetado (bounding boxes) de las placas de motocicletas.}
    \label{fig:data_acquisition}
    \vspace{2mm}
    \small
    \textit{Nota.} Captura del entorno de trabajo propio evidenciando el total de 531 ítems recolectados.
\end{figure}

\subsubsection{Actividad 2.2: Entrenamiento y optimización en Edge Impulse}
El procesamiento de los datos y la configuración del modelo de aprendizaje automático se ejecutaron mediante la plataforma \textit{Edge Impulse}. El flujo de trabajo (\textit{Impulse Design}) se configuró de la siguiente manera:
\begin{itemize}
    \item \textbf{Bloque de Imagen:} Las imágenes se redimensionaron a una resolución de 96x96 píxeles en escala de grises (\textit{Grayscale}). Esta decisión técnica es crucial para disminuir drásticamente el consumo de memoria RAM del microcontrolador sin perder los contrastes necesarios para leer una placa.
    \item \textbf{Bloque de Detección de Objetos:} Se seleccionó la arquitectura \textbf{FOMO (Faster Objects, More Objects)}. Este algoritmo es altamente eficiente para dispositivos de borde, permitiendo detecciones rápidas con una huella de memoria mínima.
\end{itemize}
Finalmente, el modelo fue cuantizado (convirtiendo los pesos de punto flotante de 32 bits a enteros de 8 bits - Int8), lo que asegura que el archivo resultante sea lo suficientemente ligero para ejecutarse en el ESP32-CAM.

% Pega esto justo debajo del texto de la Actividad 2.2
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Create_impulse.png} 
    \caption{Configuración del bloque de procesamiento y detección de objetos (FOMO) optimizado para el microcontrolador.}
    \label{fig:create_impulse}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{diagrama_flujo.png} 
    \caption{Algoritmo lógico de reconocimiento y control de acceso.}
    \label{fig:diagrama_flujo}
    \vspace{2mm}
    \small
    \textit{Nota.} Elaboración propia (2026), en Lucidchart.
\end{figure}

% --- RESULTADOS Y ANÁLISIS ---
\section{Resultados Preliminares: Entrenamiento del Modelo TinyML}

\subsection{Análisis de las métricas del modelo}
Luego del proceso de diseño y entrenamiento en la plataforma Edge Impulse, el modelo de inteligencia artificial fue evaluado utilizando el conjunto de datos de validación (\textit{Validation Set}). Las métricas obtenidas con el modelo cuantizado (Int8), el cual es fundamental para garantizar su futura ejecución dentro de las restricciones de memoria del microcontrolador ESP32-CAM, son las siguientes:

\begin{itemize}
    \item \textbf{F1-Score:} 0.90 (90\%)
    \item \textbf{Precisión (Precision):} 0.86 (86\%)
    \item \textbf{Sensibilidad (Recall):} 0.94 (94\%)
\end{itemize}

% Pega esto en la sección de Resultados, después de la viñeta de Sensibilidad (Recall)
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{Object_detection.png} 
    \caption{Métricas de validación y matriz de confusión del modelo cuantizado (Int8).}
    \label{fig:object_detection_metrics}
    \vspace{2mm}
    \small
    \textit{Nota.} Se observa un F1-Score del 90.4\% y el consumo proyectado de memoria RAM (119.4K) que certifica su viabilidad para el ESP32-CAM.
\end{figure}

% Pega esto justo debajo de la figura de la matriz de confusión (Object_detection.jpg)
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Training_graphs.png} 
    \caption{Curva de pérdida (Loss) durante el entrenamiento de la red neuronal a lo largo de 60 épocas.}
    \label{fig:training_loss}
    \vspace{2mm}
    \small
    \textit{Nota.} La convergencia simultánea y estabilización de las curvas de entrenamiento y validación demuestra un aprendizaje estable, descartando el fenómeno de sobreajuste (overfitting).
\end{figure}

Para alcanzar estos resultados y mitigar el riesgo de sobreajuste (\textit{overfitting}), se implementó la técnica de \textit{Data Augmentation}. Esta estrategia consistió en aplicar ruido digital de forma aleatoria, variaciones de brillo y recortes (\textit{cropping}) a las imágenes originales, permitiendo que la red neuronal generalice las características geométricas de las placas ante diversas condiciones lumínicas.

\subsection{Validación lógica y próximos pasos}
Los resultados obtenidos en esta fase validan matemáticamente la arquitectura algorítmica del sistema. El F1-Score del 90\% demuestra que el modelo de detección de objetos (\textit{FOMO - Faster Objects, More Objects}) es viable para la tarea de reconocimiento vehicular utilizando imágenes de baja resolución.

Como siguiente paso dentro de la ruta metodológica, se proyecta la fase de despliegue en el hardware (\textit{Deployment}). Esto consistirá en exportar el modelo cuantizado como una librería de C++ para ser integrado y ejecutado directamente en el microcontrolador ESP32-CAM. Una vez cargado el \textit{firmware}, se procederá a realizar las pruebas físicas de funcionamiento en tiempo real para medir variables como la latencia de inferencia y la correcta activación de la etapa electrónica de señalización.

% --- PRESUPUESTO DEL PROTOTIPO ---
\section{Presupuesto del Prototipo}
Para la ejecución del Producto Mínimo Viable (MVP), se ha diseñado un presupuesto optimizado que refleja la viabilidad económica del proyecto. A diferencia de los sistemas comerciales, este prototipo utiliza componentes de bajo costo y alta eficiencia. En la Tabla \ref{tab:presupuesto_mvp} se detallan los costos de los insumos principales:

\begin{table}[!ht]
\centering
\caption{Presupuesto estimado para el prototipo funcional (MVP)}
\label{tab:presupuesto_mvp}
\renewcommand{\arraystretch}{1.5}
\begin{tabularx}{\columnwidth}{>{\raggedright\arraybackslash}p{0.45\columnwidth} >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\raggedright\arraybackslash\bfseries}X}
\toprule
Componente & Cantidad & Valor Unitario & Valor Total \\
\midrule
Microcontrolador ESP32-CAM (con cámara) & 1 & \$ 60,000 & \$ 60,000 \\
Sensor Infrarrojo E18-D80NK & 1 & \$ 22,000 & \$ 22,000 \\
Módulo Relé de 1 canal (5V) & 1 & \$ 8,500 & \$ 8,500 \\
Placa Fenólica y conectores & 1 & \$ 15,000 & \$ 15,000 \\
Insumos Impresión 3D (Filamento PLA) & 1 & \$ 20,000 & \$ 20,000 \\
Fuente de poder 5V/2A & 1 & \$ 18,000 & \$ 18,000 \\
\midrule
\multicolumn{3}{r}{\textbf{Inversión Total Prototipo}} & \textbf{\$ 143,500} \\
\bottomrule
\end{tabularx}
\end{table}
...
El desarrollo del proyecto se proyecta en un periodo de 8 semanas, distribuidas para cubrir desde la adquisición de datos hasta la validación final. La Tabla \ref{tab:cronograma} muestra la distribución temporal de las fases metodológicas:

\begin{table}[H]
\centering
\caption{Cronograma de ejecución del proyecto (Semanas)}
\label{tab:cronograma}
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\columnwidth}{X | c | c | c | c | c | c | c | c}
\toprule
\textbf{Actividad / Semana} & \textbf{1} & \textbf{2} & \textbf{3} & \textbf{4} & \textbf{5} & \textbf{6} & \textbf{7} & \textbf{8} \\
\midrule
